...
while True:
    ...
    # Draw recangle around face
    for (x, y, w, h) in faces:
        cv2.rectangle(frame, (x, y), (x+w, y+h), (164, 189, 25), 2)  # blue rectangle with thickness 2
    
    # Display the count of faces
    font = cv2.FONT_HERSHEY_COMPLEX
    cv2.putText(frame, f"people count: {len(faces)}", (10, 30), font, 1, (132, 25, 189), 2, cv2.LINE_AA)

    # Display resulting frame with face detection and people count
    cv2.imshow("Face tracking with and counting - press q for quit", frame)
